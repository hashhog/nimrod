...
type
  ...
  AssumeutxoData* = object
    ...
    chainwork*: array[32, byte]      ## nChainWork at the base; zeros if unknown
    baseTailHeaders*: seq[BlockHeader]
      ## Ascending 80-byte headers ending at the base (last hash == blockhash).
      ## Empty when the campaign entry carries no ancestry. Snapshot-boot
      ## persists this band so the header chain can start at the assumeUTXO
      ## base instead of genesis (MTP + retarget ancestors + getheaders).
...
proc parseCampaignHeaderHex*(hex: string): BlockHeader =
  ## Decode 160 hex chars of the 80-byte header serialization (wire order,
  ## not a reversed hash). Campaign `base_header` / `base_tail_headers`.
  if hex.len != 160:
    raise newException(ValueError, "header hex must be 160 characters, got " & $hex.len)
  var raw = newSeq[byte](80)
  for i in 0 .. 79:
    raw[i] = byte(parseHexInt(hex[i*2 ..< i*2 + 2]))
  deserializeBlockHeader(raw)

proc campaignHeaderHash*(header: BlockHeader): BlockHash =
  ## Double-SHA256 of the 80-byte header serialization (internal byte order).
  BlockHash(doubleSha256(serialize(header)))
...
proc mainnetParams*(): ConsensusParams =
  ...
  result.checkpoints = @[
    ...
    (105000'u32, BlockHash(hexToBytes32(
        "00000000000291ce28027faea320c8d2b054b2e0fe44a773f3eefb151d6bdc97"))),
    (134444'u32, BlockHash(hexToBytes32(
        "00000000000005b12ffd4cd315cd34ffd4a594f430ac814c91184a0d42d2b0fe"))),
    (168000'u32, BlockHash(hexToBytes32(
        "000000000000099e61ea72015e79632f216fe6cb33d7899acb35b75c8303b763"))),
    (193000'u32, BlockHash(hexToBytes32(
        "000000000000059f452a5f7340de6682a977387c17010ff6e6c3bd83ca8b1317"))),
    (210000'u32, BlockHash(hexToBytes32(
        "000000000000048b95347e83192f69cf0366076336c639f9b7228e9ba171342e"))),
    (216116'u32, BlockHash(hexToBytes32(
        "00000000000001b4f4b433e81ee46494af945cf96014816a4e2370f11b23df4e"))),
    (225430'u32, BlockHash(hexToBytes32(
        "00000000000001c108384350f74090433e7fcf79a606b8e797f065b130575932"))),
    (250000'u32, BlockHash(hexToBytes32(
        "000000000000003887df1f29024b06fc2200b55f8af8f35453d7be294df2d214"))),
    (279000'u32, BlockHash(hexToBytes32(
        "0000000000000001ae8c72a0b0c301f67e3afca10e819efa9041e458e9bd7e40"))),
    (295000'u32, BlockHash(hexToBytes32(
...
proc loadCampaignAssumeutxo*(params: var ConsensusParams) =
  ...
  ##       "m_chain_tx_count": N,
  ##       "base_header"?: 160-hex, "base_tail_headers"?: [160-hex, ...],
  ##       "chainwork"?: 64-hex } ]
  ##
  ...
  for entry in root.elems:
    ...
    let blockhash = BlockHash(hexToBytes32(blockhashHex))

    # Optional snapshot-base ancestry. Validated structurally here (well-formed
    # hex, 80-byte headers, prev-hash linkage, last header hashes to this
    # entry's blockhash). Proof-of-work is re-checked when the band is grafted
    # into the header index. Missing ancestry is allowed — legacy fixtures
    # stay loadable, they just cannot present the snapshot tip.
    var tailHeaders: seq[BlockHeader] = @[]
    var rawTails: seq[string] = @[]
    if entry.hasKey("base_tail_headers") and
        entry["base_tail_headers"].kind == JArray and
    ...
    var chainwork: array[32, byte]
    if entry.hasKey("chainwork") and entry["chainwork"].kind == JString:
      var cw = entry["chainwork"].getStr
...
proc buildGenesisBlock*(params: ConsensusParams): Block =
  ...
  let genesisOutputScript = @[
    ...
    0xa6, 0x71, 0x30, 0xb7,
    0x10, 0x5c, 0xd6, 0xa8, 0x28, 0xe0, 0x39, 0x09, 0xa6, 0x79, 0x62, 0xe0,
    0xea, 0x1f, 0x61, 0xde,
...
proc buriedDeploymentActive*(activationHeight: int,
