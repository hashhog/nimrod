...
# WHY THIS EXISTS
# ---------------
# `nimble test` builds and runs tests/test_all.nim, which imports 110 of the
# 222 test_*.nim files in tests/. The other 112 are never referenced, so they
# are never compiled and never run. Until 2026-08-30 test_all.nim did not even
# compile (an ambiguous MinBlocksToKeep, fixed in 62a042a), so the aggregate ran
# ZERO tests -- every nimrod "tests pass" on record came from running individual
# files by hand.
#
# Pass 1 is the aggregate. Pass 2 compiles and runs EVERY test file on its own,
# which is the only way to see the true denominator; it reports compile failures
# separately from test failures, because a file that no longer compiles is a
# different problem from one that fails.
#
# Pass 2 is SLOW -- it is one nim compile per file (~10-30s each, so roughly an
# hour for 222). It is a periodic check, not a pre-commit gate. Limit it while
# iterating:  ./run-all-tests.sh --only 'test_addr*'
set -uo pipefail
cd "$(dirname "$0")"
FILTER="${2:-}"; [ "${1:-}" = "--only" ] && FILTER="$2"

echo "== pass 1: aggregate (tests/test_all.nim -- 110 imported modules)"
